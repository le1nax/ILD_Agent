...
def _build_messages(state: State) -> list:
    ...
    if is_complex:
        system_text = (
            ...
            "   c) Recommended workup. This section MUST explicitly address "
            "whether a lung biopsy is required for this case — never omit "
            "this question. Choose ONE of:\n"
            "      • NOT required — state which guideline criteria allow a "
            "confident diagnosis without histology (e.g., typical UIP pattern "
            "on HRCT in the appropriate clinical context per the IPF "
            "guideline).\n"
            "      • REQUIRED — specify the recommended modality (transbronchial "
            "lung cryobiopsy / TBLC, surgical lung biopsy / SLB, or transbronchial "
            "forceps biopsy / TBLB) and cite the guideline criteria that trigger "
            "it (e.g., probable UIP, indeterminate for UIP, alternative diagnosis "
            "suspected, fibrotic HP differential).\n"
            "      • GUIDELINE-SILENT or MDD JUDGEMENT — only when the cited "
            "passages genuinely do not provide a recommendation for this "
            "scenario; say so explicitly and explain what is missing.\n"
            "      Always cite the source(s) supporting your biopsy decision.\n"
